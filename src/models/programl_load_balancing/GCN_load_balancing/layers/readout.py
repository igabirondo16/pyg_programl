...
class Readout(nn.Module):
    ...
    def __init__(
        ...
        if self._discretize_problem:
            self.feed_forward = nn.Sequential(
                nn.Linear(2 * hidden_size + aux_features_dim, graph_x_layer_size),
                nn.ReLU(),
        ...
        self.batch_normalization = nn.BatchNorm1d(2 * hidden_size + aux_features_dim)
    ...
    def forward(
        ...
        initial_graph_readout = global_mean_pool(initial_node_states, graph_nodes_list)
        graph_readout = global_mean_pool(final_node_states, graph_nodes_list)

        # Concatenate the auxiliar variables
        extended_graph_readout = torch.cat(
            (graph_readout, initial_graph_readout, aux_variables), dim=1
        )
