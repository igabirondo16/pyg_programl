...
class ProgramlDevMapModel(nn.Module):
    """Replica of the ProGraML model for the heterogeneous device
    mapping problem.
    ...
    def __init__(
        ...
        Args:
            ...
            dropout_rate_update_layer (float): Probability of using Dropout
                in the update layer. Notice that if
                dropout_rate_update_layer == 0.0 it has no regularization effect.
    ...
    def forward(self, batch_data: Batch) -> torch.Tensor:
        ...
        logits = self.readout(
            initial_node_states, node_states, aux_features, num_graphs, graph_nodes_list
        )
