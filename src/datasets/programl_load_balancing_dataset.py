...
class ProgramlLoadBalancingDataset(InMemoryDataset):
    """
    Class to load the heterogeneous system's load balancing dataset.
    ...
    def __init__(
        ...
        discretize_problem: bool = True,
    ):
        """Constructor of the load balancing dataset.
        ...
        Args:
            ...
            discretize_problem (bool, optional): Load the dataset in regression or classification problem.
                Defaults to True.
        """
        self.nvidia_labels_count = {0: 0, 1: 0, 2: 0, 3: 0, 4: 0}

        self.amd_labels_count = {0: 0, 1: 0, 2: 0, 3: 0, 4: 0}

        self.discretize_problem = discretize_problem

        self.labels = torch.tensor([0.0, 25.0, 50.0, 75.0, 100.0], dtype=torch.float32)
    ...
    def processed_file_names(self):
        ...
        Files:
        - data_nvidia_*.pt: File containing all the HeteroGraphs corresponding to the Nvidia dataset.
        - data_amd_*.pt: File containing all the HeteroGraphs corresponding to the Amd dataset.
        """

        if self.discretize_problem:
            return ["data_nvidia_lb_discr.pt", "data_amd_lb_discr.pt"]

        else:
            return ["data_nvidia_lb_cont.pt", "data_amd_lb_cont.pt"]
    ...
    def __load_dataframe(self, df_path: str) -> pd.DataFrame:
        ...
        # IMPORTANT:
        # We are creating a regression problem, in this case the label
        # is a proportion that we want to predict
        runtime_cpu = df["runtime_cpu"]
        runtime_gpu = df["runtime_gpu"]

        # Compute the proportion of work given to the cpu
        factor = runtime_gpu / runtime_cpu
        cpu_percentage = round(factor / (1 + factor), 2)  # Round to 2 decimals
        cpu_percentage *= 100.0  # Convert ratio to percentage

        """
        transfer_bytes_log1p = np.log1p(df["transfer"].to_numpy())
        wgsize_log1p = np.log1p(df["transfer"].to_numpy())

        #transfer_bytes_log1p = df["transfer"].to_numpy()
        #wgsize_log1p = df["transfer"].to_numpy()

        transfer_bytes_log1p =transfer_bytes_log1p.reshape(-1,1)
        wgsize_log1p = wgsize_log1p.reshape(-1,1)

        power_scaler = PowerTransformer()
        standard_scaler = StandardScaler()
        min_max_scaler = MinMaxScaler(feature_range=(-1,1))

        transfer_bytes1 = power_scaler.fit_transform(transfer_bytes_log1p)
        transfer_bytes2 = standard_scaler.fit_transform(transfer_bytes1)
        transfer_bytes_log1p = min_max_scaler.fit_transform(transfer_bytes2)

        wgsize1 = power_scaler.fit_transform(wgsize_log1p)
        wgsize2 = standard_scaler.fit_transform(wgsize1)
        wgsize_log1p = min_max_scaler.fit_transform(wgsize2)
        """
    ...
    def __create_dataframe_graphs(
        ...
        for _, row in df.iterrows():
            ...
            # IMPORTANT:
            # Notice that the label is a proportion!! (Hence, it's a float)
            graph.features.feature["devmap_label"].float_list.value[:] = [row["label"]]
    ...
    def __programl_to_heterograph(
        self, programl_graph_path: str, vocabulary: dict, is_nvidia_graph: bool
    ) -> HeteroData:
        """
        ...
        Args:
            ...
            is_nvidia_graph (bool): Whether the graph is part of the nvidia dataset.
        """
        ...
        # IMPORTANT: Notice that the label is a proportion!!
        cpu_percentage = programl_graph.features.feature[
            "devmap_label"
        ].float_list.value[0]
        ...
        # Store it for the regression problem
        label = cpu_percentage

        # In case it is a classification problem
        if self.discretize_problem:
            aux = torch.full(size=[5], fill_value=cpu_percentage)
            label = torch.abs(self.labels - aux).argmin()

        # Count the labels for checking the total distribution of classes
        if self.discretize_problem:
            if is_nvidia_graph:
                self.nvidia_labels_count[label.item()] += 1

            else:
                self.amd_labels_count[label.item()] += 1
    ...
    def __create_heterograph_dataset(
        self, input_path: str, vocabulary: dict, is_nvidia_graph: bool
    ) -> List[HeteroData]:
        """
        ...
        Args:
            ...
            vocabulary (dict): Vocabulary dictionary for the ProGraML tokens.
            is_nvidia_graph (bool): Whether the graph is part of the nvidia dataset.
        ...
        for graph_name in os.listdir(input_path):
            ...
            hetero_graph = self.__programl_to_heterograph(
                graph_path, vocabulary, is_nvidia_graph
            )
    ...
    def process(self):
        ...
        nvidia_heterographs = self.__create_heterograph_dataset(
            nvidia_graph_path, programl_vocab, is_nvidia_graph=True
        )
        amd_heterographs = self.__create_heterograph_dataset(
            amd_graph_path, programl_vocab, is_nvidia_graph=False
        )
        ...
        print(f"Number of NVIDIA graphs: {len(nvidia_heterographs)}")
        print("Distribution of classes in the NVIDIA dataset:")
        print(self.nvidia_labels_count)

        print()
        print(f"Number of AMD graphs: {len(amd_heterographs)}")
        print("Distribution of classes in the AMD dataset:")
        print(self.amd_labels_count)
