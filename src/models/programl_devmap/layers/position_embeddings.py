...
"""
LAYER COPIED FROM ORIGINAL ProGraML work by Chris Cummins.
...
class PositionEmbeddings(nn.Module):
    ...
    def forward(self, positions, demb, dpad: int = 0):
        ...
        """
        inv_freq = 1 / (10000 ** (torch.arange(0.0, demb, 2.0) / demb))  # dim?

        sinusoid_inp = torch.ger(positions, inv_freq)  # num_positions x num_inv_freq
        pos_emb = torch.cat(
            (torch.sin(sinusoid_inp), torch.cos(sinusoid_inp)), dim=1
        )  # num_positions x (2 * num_inv_freq)
        ...
        return pos_emb  # num_positions x ((2 * num_inv_freq) + 2)
