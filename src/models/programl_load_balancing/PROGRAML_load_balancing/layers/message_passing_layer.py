...
from torch_geometric.nn import HeteroDictLinear, MessagePassing
...
class MessagePassingLayer(MessagePassing):
    ...
    def __init__(
        ...
        self.messages_net = HeteroDictLinear(
            in_channels={
                "control":self.__hidden_size,
                "data":self.__hidden_size,
                "call":self.__hidden_size
            },
            out_channels=self.__hidden_size,
            types=["control","data","call"]
        )
        """

        self.gru = UpdateLayer(self.__hidden_size, dropout_rate_update_layer)

        self.update_layer = LinearLayer(
    ...
    def reset_parameters(self):
        ...
        # self.control_net.reset_parameters()
        # self.data_net.reset_parameters()
        # self.call_net.reset_parameters()
    ...
    def forward(
        ...
        # messages = self.messages_net(temp)

        # control_node_states = messages.get("control")
        # data_node_states = messages.get("data")
        # call_node_states = messages.get("call")

        # control_node_states = self.control_net(node_states)
        # data_node_states = self.data_net(node_states)
        # call_node_states = self.call_net(node_states)

        # transformed_node_states = torch.stack([control_node_states, data_node_states, call_node_states], dim=0)
        transformed_node_states = self.linear_net(node_states)

        # Create position embeddings
        pos_gating = 2 * torch.sigmoid(self.pos_transform(self.position_embs))
        # pos_gating = self.relu(pos_gating)
        ...
        # print("GPU after reshape: " + str(torch_geometric.profile.get_gpu_memory_from_nvidia_smi()))
        ...
        # print("GPU after messages: " + str(torch_geometric.profile.get_gpu_memory_from_nvidia_smi()))
    ...
    def message(
        ...
        # print("GPU before creatign messages: " + str(torch_geometric.profile.get_gpu_memory_from_nvidia_smi()))
        # Perform a single call to F.embedding()
        messages = F.embedding(edge_sources, transformed_node_states)

        # print("GPU after creatign messages: " + str(torch_geometric.profile.get_gpu_memory_from_nvidia_smi()))
        ...
        # print("GPU after factor: " + str(torch_geometric.profile.get_gpu_memory_from_nvidia_smi()))
    ...
    def update(
        ...
        """
        nodes = torch.stack([aggr_out, node_states], dim=1)
        nodes = nodes.view(
            -1, self.__hidden_size * 2
        )
        update_output = self.update_layer(nodes)
        update_output = self.relu(update_output)
        return update_output
        """
